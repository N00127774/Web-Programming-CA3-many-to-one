<!doctype html>
<html>
    <head>
        <meta charset="utf-8">
        <title>My Website</title>
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <!-- Bootstrap -->
        <link href="css/bootstrap.min.css" rel="stylesheet">
        <link href="css/custom.css" rel="stylesheet">
        <script src="js/respond.js"></script>
        <link href="css/style.css" rel="stylesheet">
        <link href="css/font-awesome.css" rel="stylesheet">
        <link href="css/font-awesome.min.css" rel="stylesheet">
        <link href="styles/ihover.css" rel="stylesheet"> <!-- for the images and effects on them-->
     
        <link href='http://fonts.googleapis.com/css?family=Lato:400,900|Kreon' rel='stylesheet' type='text/css'>

    </head>

    <body>
        



<nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
		<!-- Dropdown Menu for Mobile Devices -->
		<div class="container-fluid">
			<div class="navbar-header">
				<button type="button" class="navbar-toggle" data-toggle="collapse" data-target="#collapse">
					<span class="sr-only">Toggle Navigation</span>
					<span class="icon-bar"></span>
					<span class="icon-bar"></span>
					<span class="icon-bar"></span>
				</button>
                            <a class="navbar-brand" href="index.html">  <img src= "img/piechart.png" class="img-responsive BigLogo " alt=""></a>
			</div>
							
			<div class="collapse navbar-collapse" id="collapse">
				<div class="container-fluid">
					<ul class="nav navbar-nav navbar-right">
						<li><a href="#">Home <span class=" glyphicon glyphicon-home"></span></a></li>
						<li><a href="#">About US <span class=" glyphicon glyphicon-user "></span></a></li>
						<li><a href="#">Sign Up <span class=" glyphicon glyphicon-pencil "></span></a></li>
                                                <li><a href="#">Contact Us <span class=" glyphicon glyphicon-envelope "></span></a> </li>
                                                <li><a href="login.html">Login <span class="glyphicon glyphicon-user"></span></a></li>
					</ul>
				</div>
			</div>
		</div>
	</nav>
           




        <div class="jumbotron">	<!-- Jumbotron -->
            <div class="container"><!-- container -->
                <div class="row">
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <center>
                            <h1>Asset Management Agency </h1>
                            <p><a class="btn btn-primary btn-lg" href="#" role="button">Learn more</a></p>
                        </center>
                    </div>
                </div>
            </div><!-- End of container-->
        </div><!-- End of Jumbotron-->

<!--<img src="img/jumbotron.jpg" alt="" class="bigImage">-->



        <div class="container">
            <hr></hr>
            <div class="row Topbody"><!-- Start of row2 with text -->
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="text-center">
                        <h1> Who are we?</h1>
                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                            <blockquote><p>Here in our Company , our customers come first, </p>
                                <p>we make their problems ours, and we come up with solutions together.
                                    That is why we are Ireland's leading Asset Management Company.</p></blockquote>
                        </div>
                    </div>
                </div>


            </div><!-- End of row2 with text -->



            <div class="row Services"><!--Our service row  with  Our service text -->
                <div class="col-lg-12 col-md-12 col-xs-12">

                    <h1 class ="text center"> Our Services</h1> 
                </div>

            </div><!-- end of Our service row  with  Our service text -->
            <div class="row  AboutServices">

                <div class="col-lg-4">
                    <div class="thumbnail ">
                        <img src= "img/share21.png" class="img-responsive icons " alt="">

                        <h2 class="center greytxt"> Shares </h2>
                        <div class="text-center">
                            <p class="greytxt"> The very qualities which recommend it to outdoor signage — ease of reading at a distance, and features durable 
                                enough to withstand the elements make Gotham an excellent choice for small sizes as well.</p>

                            <button type="button" class="btn btn-Share ">Read More</button>


                        </div>
                    </div>
                </div>


                <div class="col-lg-4">
                    <div class="thumbnail ">
                        <img src="img/house12.png" class="img-responsive icons" alt="">
                        <h2 class="center greytxt"> Shares </h2>
                        <div class="text-center">
                            <p class="greytxt"> The very qualities which recommend it to outdoor signage — ease of reading at a distance, and features durable 
                                enough to withstand the elements make Gotham an excellent choice for small sizes as well.</p>

                            <button type="button" class="btn btn-Share ">Read More</button>


                        </div>
                    </div>
                </div>

                <div class="col-lg-4">
                    <div class="thumbnail ">
                        <img src="img/stats9.png" class="img-responsive icons"alt="">
                        <h2 class="center greytxt"> Stocks </h2>
                        <div class="text-center">
                            <p class="greytxt"> The very qualities which recommend it to outdoor signage — ease of reading at a distance, and features durable 
                                enough to withstand the elements make Gotham an excellent choice for small sizes as well.</p>

                            <button type="button" class="btn btn-Share ">Read More</button>

                        </div>
                    </div>
                </div>
                <hr></hr>
            </div>
        </div>


        <div class="container-fluid teamcontainer">
            <div class="row AboutOurTeam">

                <div class="text-center ">
                    <h2 class="text-center"><span class=" glyphicon glyphicon-arrow-left "></span>About Our Team <span class=" glyphicon glyphicon-arrow-right "></span></h2>
                </div>



                <!--Start of the rounded images wioth text under-->
                <div class="col-lg-4 Asset transparent">
                    <div class="thumbnail">

                        <center>
                            <div class="ih-item circle effect1"><a href="#">

                                    <div class="spinner"></div>

                                    <div class="img img-responsive"><img src="img/sectwo.jpg" alt="img" class="ImageMargin"></div>

                                    <div class="info">
                                        <div class="info-back">
                                            <h3>Get in contact</h3>
                                            <p> jamesRichburg@yahoo.com</p>


                                        </div>
                                    </div></a></div>
                            <h5 class ="imageMargTextMargin-top"><a>Jason  Richburg</a></h5>
                            <h5 class="JobSpecs"> Secretary</h5>
                            <p> Jason is the Secretary of this branch,Answers the call in the office, and can also register new customers to the company,
                                Gets all the information about a new customer for them to register. </p>
                        </center>
                        <!-- end normal -->
                    </div>
                </div>


                <div class="col-lg-4 Asset transparent">
                    <div class="thumbnail">
                        <!-- normal -->
                        <center>
                            <div class="ih-item circle effect1"><a href="#">

                                    <div class="spinner"></div>

                                    <div class="img img-responsive"><img src="img/aboutOurTeam/guy.jpg" alt="img"></div>

                                    <div class="info">
                                        <div class="info-back">
                                            <h3>Get in contact</h3>
                                            <p class="email"> BrianZimmerman@yahoo.ie</p>
                                        </div>
                                    </div></a></div>
                            <h5 class ="imageMargTextMargin-top"><a>Brian Zimmerman</a></h5>
                            <h5 class="JobSpecs"> Asset Manager </h5>
                            <p>	Brian is the Asset Manager this is the person in-charge of the Company asset, the asset manager  
                                The Asset Manager will use the system to keep files of customer’s assets, any documentation and any other information about them, 
                                like your transactions	</p>

                        </center>

                        <!-- end normal -->
                    </div>
                </div>





                <div class="col-lg-4 Asset transparent">
                    <div class="thumbnail">
                        <!-- normal -->						<center>
                            <div class="ih-item circle effect1"><a href="#">
                                    <div class="spinner"></div>
                                    <div class="img  "><img src="img/Branch.jpg " alt="img" class="ImageMargin"></div>
                                    <div class="info">
                                        <div class="info-back">
                                            <h3>Get in contact</h3>
                                            <p>ErnestFesco@gmail.com</p>
                                        </div>
                                    </div></a></div>
                            <h5 class ="imageMargTextMargin-top"><a>Ernest Fesco  Obi</a></h5>
                            <h5 class="JobSpecs">Branch Manager </h5>
                            <p>	 Ernest is the Branch Manager will be the person that checks different branches of the company, 
                                who makes sure that all of the different branches are financially stable. The branch Manager will be able to use the system to its full potential. 	</p>

                        </center>
                    </div>
                </div>
            </div>
        </div>
        <!-- end normal -->		
        <div class="container ">

            <div class="row InternshipSec">

                <div class="col-lg-8 col-md-8 col-sm-8 col-xs-8 LAPPYiMAGE">
                    <img src="img/lappyimage2.png" alt="" class="img-responsive moveleft ">
                </div>

                <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4  text-center ">
                    <h2 class="IntershipText greytxt"> Are you looking for Internships!</h2 >
                    <p class="greytxt"> Do you want to gain experience or work with us? becuase we are an awesome
                        Agency! If you  want to be part of us, you have to be hardworking,trustworthy and fully flexible.</p>
                    <button type="button" class="btn btn-contact">Contact us</button>
                </div>


            </div>


            <!-- big Image Part-->
            <div class="container">
                <div class= "col-lg-6">
                    <img src="img/bigimage2.png" alt="" class="img-responsive push-right ">
                </div>

                <div class= "col-lg-6">
                    <img src="img/bigimage2.png" alt="" class="img-responsive circle-image  ">
                </div>

            </div>
        </div>






        <div class="container-fluid footer">
            <div class="footer">
                <div class="row">
                    <div class="col-lg-4">
                        <h2>Contact Information <span class=" glyphicon glyphicon-phone-alt"></h2>
                        <li> Some Address 987,NY</li>
                        <li> <span class=" glyphicon glyphicon-earphone"> tel:00353439359894</li>
                        <li> <span class=" glyphicon glyphicon-phone"> tel:00353439359894</li>
                        <li> <span class="glyphicon glyphicon-folder-close"></span> AsssetManagementagency@yahoo.ie</li>
                        <li> <span class="glyphicon glyphicon-briefcase" ></span>@Asset_co</li>
                        <li>  <span class=" glyphicon glyphicon-lock "></span> BaabatundeAgency</li> 
                    </div>

                    <div class="col-lg-5">
                        <h2>Newsletter <span class=" glyphicon glyphicon-list "></h2>
                        <p> Register to our newsletter and be updated with the latests information regarding our services, offers
                            and much more.
                        <form class="form-horizontal" role="form">
                            <div class="form-group">
                                <label class="control-label col-sm-2" for="email">Email:</label>
                                <div class="col-sm-8">
                                    <input type="email" class="form-control" id="email" placeholder="Enter email">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="control-label password col-xs-2" for="pwd">Password:</label>
                                <div class="col-sm-8 ">          
                                    <input type="password" class="form-control " id="pwd" placeholder="Enter password">
                                </div>
                            </div>
                            <div class="form-group">        
                                <div class="col-sm-10">
                                    <div class="checkbox">
                                        <label><input type="checkbox"> Remember me</label>
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">        
                                <div class=" col-sm-12">
                                    <button type="submit" class="btn btn-submit">Submit</button>
                                </div>
                            </div>
                        </form>
                    </div>


                    <div class="col-lg-3">
                        <h2>Opening Hours  <span class=" glyphicon glyphicon-time "></h2>
                        <li> Monday 9:00am-5:00pm</li>
                        <li> Tuesday 9:00am-5:00pm</li>
                        <li> Wednesday9:00am-5:00pm</li>
                        <li> Thursday 9:00am-5:00pm</li>
                        <li> Friday 9:00am-5:00pm</li>

                    </div>

                </div>

            </div>




        </div>
 























        <!-- javascript -->
        <script src="http://code.jquery.com/jquery-latest.min.js"></script>
        <script src="js/bootstrap.min.js"></script>
    </body>
</html>
